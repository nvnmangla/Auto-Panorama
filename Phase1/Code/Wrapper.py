...
def ANMS(img, best):
    ...
    Args:
        ...
        best (int): Best number of corners

    Returns:
        np.array : best sorted corner array [ED,xi,yi]
    """
    print("Performing ANMS")
    img2 = copy.deepcopy(img)
    gray = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    gray = np.float32(gray)
    dst = cv2.goodFeaturesToTrack(gray,1000, 0.1,10)
...
def fdis(img, best, patch_size):
    """
    Feature Descriptor

    Args:
        img (cv::Mat): 3 channel image
        best (int): number of best points 
        patch_size (int) dimention of feature patch

    Returns:
        list , list: set of features,set of points 
    """
    ...
    for i in range(X.shape[0]):
        ...
        if (mid < x0 < gray.shape[0]-mid and mid < y0 < gray.shape[1]-mid):
            ...
            res = cv2.resize(
                dst, (8, 8), interpolation=cv2.INTER_LINEAR)

            cv2.imwrite('solution/features/feature'+
                        str(i)+'.png', patch)
            res_vector = np.reshape(res, (64, 1)) # vecorizing patch
            res_vector = res_vector-np.mean(res_vector)  # Normalising vector
            res_vector = res_vector / (np.std(res_vector))
            point = (x0, y0)
            points.append(point) # set of points 
            sample.append(res_vector) #set of features 
...
def match_features(im1, im2, best):
    """
    Finding feature corres[ondance between 2 images]

    Args:
        im1 (cv::Mat): image 1 
        im2 (cv::Mat): image 2
        best (int): number of best coreners

    Returns:
        np.array,flag: Feature pair set, flag for feature match
    """
    size = 41 ## Taking patch size = 41 
    fet1, cor1 = fdis(im1, best, size)
    fet2, cor2 = fdis(im2, best, size)
    ...
    if (len(fet1) == len(cor1)) and (len(fet2) == len(cor2)):
        ...
        pair_stack = []
        for point in data:
            sorted_dis = point[np.argsort(point[:, 0])]
...
def Homography(img1, img2, best, thresh, N):
    pairs, flag = match_features(img1, img2, best)
    if flag:
        model, status = RANSAC(pairs, thresh, N)
        im12 = draw(model, img1, img2)
        cv2.imwrite("solution/RANSAC.png", im12)
        print(model[0].shape)
...
def RANSAC(pairs, thresh, N):
    ...
    for n in range(N):
        ...
        for i in range(len(pairs)):
            pt_ = np.array([pairs[i][0, 0], pairs[i][0, 1], 1])
            expected = H@pt_ 
            if expected[2] == 0:
        ...
        if new < len(iliners):
            ...
            model = np.array(pairs)[iliners]
            print(model.shape)
    if model.shape[0] < 5:
        print(f"Not enough RANSAC Match {model.shape[0] } found")
        return model, False
    else:
        print(f"{model.shape[0]} found")
        return model, True
...
def Blend(images, best, thresh, N):
    img1 = images[0]
    
    for img in images[1:2]:
        H, status = Homography(img1, img, best, thresh, N)
        if not status:
            print('Cannot Stitch')
            break
        else:
...
def main():
    ...
    Parser.add_argument('--BasePath', default='../Data/Train/Set1',
                        help='Give your path')
    ...
    #             dst[j,:,1])),1, (0, 0, 255), -1)
    #     cv2.imwrite("solution/corners_"+str(i+1)+".jpg",img)
    #     img = copy.deepcopy(images[i])
    ...
    #     cv2.imwrite(f"solution/ANMS_{i+1}.png", img)
    #     img2 = copy.deepcopy(images[i+1])

    #     img = copy.deepcopy(images[i])
    #     pair,_ = match_features(img,img2,500)
    #     temp  = draw(pair,img,img2)
    #     cv2.imwrite(f"solution/match_{i}_{i+1}.png",temp)
        
    #     ran, _ = RANSAC(pair,30,3000)
    #     temp = draw(ran, img, img2)
    #     cv2.imwrite(f"solution/ransac_{i}_{i+1}.png", temp)
            
        
    mypano = Blend(images, best = 4000, thresh=20, N=10000)
    
    cv2.imwrite("solution/mypano.png", mypano)
